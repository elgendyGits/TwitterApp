...
import com.android.twitterapp.custom.tweetview.utils.TweetTextUtils;
...
public class TweetRepository {
   ...
   public TweetRepository(Handler mainHandler, SessionManager<TwitterSession> userSessionManagers) {
    ...
    public void retweet(final long tweetId, final Callback<Tweet> cb) {
        getUserSession(new LoggingCallback<TwitterSession>(cb, Twitter.getLogger()) {
            @Override
            public void success(Result<TwitterSession> result) {
    ...
    public void unretweet(final long tweetId, final Callback<Tweet> cb) {
        getUserSession(new LoggingCallback<TwitterSession>(cb, Twitter.getLogger()) {
            @Override
            public void success(Result<TwitterSession> result) {
    ...
    public void getUserSession(final Callback<TwitterSession> cb) {
     ...
     * @param cb callback
     */
    public void loadTweet(final long tweetId, final Callback<Tweet> cb) {
        ...
        twitterCore.getApiClient().getStatusesService()
                .show(tweetId, null, null, null).enqueue(new TweetRepository.SingleTweetCallback(cb));
    }
     ...
     * @param cb callback
     */
    public void loadTweets(final List<Long> tweetIds, final Callback<List<Tweet>> cb) {
        ...
        twitterCore.getApiClient().getStatusesService().lookup(commaSepIds, null, null, null)
                .enqueue(new TweetRepository.MultiTweetsCallback(tweetIds, cb));
    }
